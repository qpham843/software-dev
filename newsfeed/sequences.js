...
function scoreArticle(articleNumber, indexNumber) {
...
function createVisualization(article, articleNumber, indexNumber) {
  // Variables
    var width = 300;
    var height = 300;
    ...
    var g = d3.select("#chart" + articleNumber)
...
function createCategories(d) {
    for (var i = 0; i < d.data.children.length; i += 1) {
        ...
        curHTML = curHTML + "<div id='category" + articleNumber + i + "' class='collapsible'>" + catData.name + "</div>" + "<div id='" + catId + articleNumber + "' class='content'></div>";
        document.getElementById("categories" + articleNumber).innerHTML = curHTML;
        document.getElementById("category" + articleNumber + i).style.borderLeft = "4px solid " + colorFinder(nameToData.get(catId));
        for (var j = 0; j < catData.children.length; j += 1) {
            var catHTML = document.getElementById(catId + articleNumber).innerHTML;
            ...
            catHTML = catHTML + "<div class='collapsible'>" + subcatData.name + " </div>" + "<div id='" + subcatId + articleNumber + "' class='content'></div>";
            document.getElementById(catId + articleNumber).innerHTML = catHTML;
            document.getElementById(subcatId + articleNumber).style.backgroundColor = colorFinder(nameToData.get(subcatData.name));
            document.getElementById(subcatId + articleNumber).style.borderLeft = "2px double " + colorFinder(nameToData.get(subcatData.name));
            for (var k = 0; k < subcatData.startIndices.length; k += 1) {
                var subcatHTML = document.getElementById(subcatId + articleNumber).innerHTML;
                ...
                document.getElementById(subcatId + articleNumber).innerHTML = subcatHTML;
                }
            }
        }
    }
...
var tooltip = d3.select("#myArticle" + articleNumber).append("div")
    .attr("class", "tooltip")
    .style("opacity", 0);
